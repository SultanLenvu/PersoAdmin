...
MainWindowKernel::MainWindowKernel(QWidget* parent)
    : QMainWindow(parent)
{
...
void MainWindowKernel::showDatabaseTablePushButton_slot()
{
  ...
  emit showDatabaseTable_signal(AbstractGUI->DatabaseTableChoice->currentText(), RandomModel);
}
...
void MainWindowKernel::clearDatabaseTablePushButton_slot()
{
  ...
  emit clearDatabaseTable_signal(AbstractGUI->DatabaseTableChoice->currentText(), RandomModel);
}
...
void MainWindowKernel::transmitCustomRequestPushButton_slot()
{
  ...
  emit performCustomRequest_signal(AbstractGUI->CustomRequestLineEdit->text(), RandomModel);
}
...
void MainWindowKernel::createNewOrderPushButton_slot()
{
  ...
  if (!checkNewOrderInput()) {
    Interactor->generateErrorMessage("Некорректный ввод параметров нового заказа. ");
    return;
  }

  QSharedPointer<QHash<QString, QString>> orderParameters(new QHash<QString, QString>);
  orderParameters->insert("issuer_name", AbstractGUI->IssuerNameComboBox->currentText());
  orderParameters->insert("transponder_quantity", AbstractGUI->TransponderQuantityLineEdit->text());
  orderParameters->insert("box_capacity", AbstractGUI->BoxCapacityLineEdit->text());
  orderParameters->insert("pallet_capacity", AbstractGUI->PalletCapacityLineEdit->text());
  orderParameters->insert("full_personalization",
  ...
  orderParameters->insert("transponder_model", AbstractGUI->TransponderModelLineEdit->text());
  orderParameters->insert("accr_reference", AbstractGUI->AccrReferenceLineEdit->text());
  orderParameters->insert("equipment_class", AbstractGUI->EquipmentClassLineEdit->text());
  orderParameters->insert("manufacturer_id", AbstractGUI->ManufacturerIdLineEdit->text());
...
void MainWindowKernel::startOrderAssemblingPushButton_slot()
{
  ...
  if (AbstractGUI->OrderIdLineEdit1->text().toInt() == 0) {
    Interactor->generateErrorMessage("Некорректный ввод идентификатора заказа. ");
    return;
  }
...
void MainWindowKernel::createNewProductionLinePushButton_slot()
{
  ...
  QSharedPointer<QHash<QString, QString>> productionLineParameters(new QHash<QString, QString>);
  ...
  emit createNewProductionLine_signal(productionLineParameters, ProductionLineModel);
...
void MainWindowKernel::linkProductionLinePushButton_slot()
{
  ...
  if ((!checkNewProductionLineInput()) || (AbstractGUI->BoxIdLineEdit1->text().toInt() == 0)) {
    Interactor->generateErrorMessage("Некорректный ввод параметров производственной линии. ");
    return;
  }

  const QSharedPointer<QHash<QString, QString>> linkParameters(new QHash<QString, QString>);
...
{
  QSharedPointer<QHash<QString, QString>> linkParameters(new QHash<QString, QString>());
  MainWindowGUI* AbstractGUI = dynamic_cast<MainWindowGUI*>(CurrentGUI);
  ...
  linkParameters->insert("master_keys_type", MatchingTable->value(masterKeysType));
...
void MainWindowKernel::transponderManualReleasePushButton_slot()
{
  ...
  QSharedPointer<QHash<QString, QString>> param(new QHash<QString, QString>());
  param->insert("table", MatchingTable->value(AbstractGUI->ChoiceAnyIdComboBox->currentText()));
...
void MainWindowKernel::printTransponderStickerPushButton_slot()
{
  ...
  if (AbstractGUI->TransponderIdLineEdit->text().toUInt() == 0) {
    Interactor->generateErrorMessage("Введен некорректный идентификатор транспондера. ");
    return;
  }

  emit printTransponderSticker_signal(AbstractGUI->TransponderIdLineEdit->text(), StickerModel);
}
...
void MainWindowKernel::execStickerPrinterCommandScriptPushButton_slot()
{
  ...
  QSharedPointer<QStringList> commandScript(
      new QStringList(AbstractGUI->StickerPrinterCommandScriptInput->toPlainText().split("\n")));
...
void MainWindowKernel::applySettingsPushButton_slot()
{
  ...
  // Проверка пользовательского ввода
  if (!checkNewSettings()) {
    Interactor->generateErrorMessage("Введены некорректные данные для настроек. ");
    return;
  }
...
void MainWindowKernel::displayFirmware_slot(QSharedPointer<QFile> firmware)
{
  ...
  if (!firmware.get()->open(QIODevice::ReadOnly)) {
    Interactor->generateErrorMessage("Не удалось открыть файл прошивки для отображения.");
  }
...
void MainWindowKernel::saveSettings() const
{
  ...
  settings.setValue("log_system/display_log_enable",
			...
			? true
			: false);
  settings.setValue("log_system/file_log_enable",
  ...
  settings.setValue("log_system/udp_listen_enable",
		    AbstractGUI->LogSystemListenPersoServerCheckBox->checkState() == Qt::Checked
			? true
			: false);
  settings.setValue("log_system/udp_listen_ip", AbstractGUI->LogSystemListenIpLineEdit->text());
  settings.setValue("log_system/udp_listen_port",
		    AbstractGUI->LogSystemListenPortLineEdit->text().toInt());

  // Настройки клиента
  settings.setValue("perso_client/server_ip", AbstractGUI->PersoClientServerIpLineEdit->text());
  settings.setValue("perso_client/server_port", AbstractGUI->PersoClientServerPortLineEdit->text());

  // Настройки контроллера базы данных
  settings.setValue("postgres_controller/server_ip", AbstractGUI->DatabaseIpLineEdit->text());
  settings.setValue("postgres_controller/server_port",
		    AbstractGUI->DatabasePortLineEdit->text().toInt());
  settings.setValue("postgres_controller/database_name", AbstractGUI->DatabaseNameLineEdit->text());
  settings.setValue("postgres_controller/user_name", AbstractGUI->DatabaseUserNameLineEdit->text());
  settings.setValue("postgres_controller/user_password",
		    AbstractGUI->DatabaseUserPasswordLineEdit->text());
  settings.setValue("postgres_controller/log_enable",
  ...
  settings.setValue("sticker_printer/name", AbstractGUI->StickerPrinterNameLineEdit->text());
}
...
bool MainWindowKernel::checkNewOrderInput() const
{
  ...
  int32_t transponderQuantity = AbstractGUI->TransponderQuantityLineEdit->text().toInt();
  ...
  if ((accrReference.length() > ACCR_REFERENCE_CHAR_LENGTH) || (accrReference.length() == 0)) {
    return false;
  }

  if ((equipmnetClass.length() > EQUIPMENT_CLASS_CHAR_LENGTH) || (equipmnetClass.length() == 0)) {
    return false;
  }

  if ((manufacturerId.length() > MANUFACTURER_ID_CHAR_LENGTH) || (manufacturerId.length() == 0)) {
    return false;
  }
...
void MainWindowKernel::createTopMenuActions()
{
  ...
  connect(RequestAuthorizationGUIAct,
	  &QAction::triggered,
	  this,
...
void MainWindowKernel::createAuthorazationGUI()
{
  ...
  setGeometry(DesktopGeometry.width() * 0.1,
	      DesktopGeometry.height() * 0.1,
	      DesktopGeometry.width() * 0.2,
	      DesktopGeometry.height() * 0.1);
...
void MainWindowKernel::connectAuthorizationGUI()
{
  ...
  connect(AbstractGUI->AuthorizePushButton,
	  &QPushButton::clicked,
	  this,
...
void MainWindowKernel::connectMainWindowGUI()
{
  ...
  // База данных
  connect(AbstractGUI->ConnectDatabasePushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::connectDatabasePushButton_slot);
  connect(AbstractGUI->DisconnectDatabasePushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::disconnectDatabasePushButton_slot);
  connect(AbstractGUI->ShowDatabaseTablePushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::showDatabaseTablePushButton_slot);
  connect(AbstractGUI->InitIssuerTablePushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::initIssuerTablePushButton_slot);

  connect(AbstractGUI->TransmitCustomRequestPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::transmitCustomRequestPushButton_slot);

  // Заказы
  connect(AbstractGUI->CreateNewOrderPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::createNewOrderPushButton_slot);
  connect(AbstractGUI->StartOrderAssemblingPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::startOrderAssemblingPushButton_slot);
  connect(AbstractGUI->StopOrderAssemblingPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::stopOrderAssemblingPushButton_slot);
  connect(AbstractGUI->UpdateOrderViewPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::updateOrderViewPushButton_slot);
  connect(AbstractGUI->DeleteLastOrderPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::deleteLastOrderPushButton_slot);

  // Производственные линии
  connect(AbstractGUI->CreateNewProductionLinePushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::createNewProductionLinePushButton_slot);
  ...
  connect(AbstractGUI->LinkProductionLinePushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::linkProductionLinePushButton_slot);
  ...
  // Эмитенты
  connect(AbstractGUI->ShowIssuerTablePushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::showIssuerTablePushButton_slot);
  connect(AbstractGUI->InitTransportMasterKeysPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::initTransportMasterKeysPushButton_slot);
  connect(AbstractGUI->LinkIssuerWithKeysPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::linkIssuerWithKeysPushButton_slot);

  // Тест сервера
  connect(AbstractGUI->ReleaseTransponderPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::releaseTransponderPushButton_slot);
  connect(AbstractGUI->ConfirmTransponderPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::confirmTransponderPushButton_slot);
  connect(AbstractGUI->RereleaseTransponderPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::rereleaseTransponderPushButton_slot);
  connect(AbstractGUI->ConfirmRereleaseTransponderPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::confirmRereleaseTransponderPushButton_slot);
  connect(AbstractGUI->ProductionLineRollbackPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::productionLineRollbackPushButton_slot);

  connect(AbstractGUI->PrintBoxStickerOnServerPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::printBoxStickerOnServerPushButton_slot);
  ...
  connect(AbstractGUI->TransponderManualRefundPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::transponderManualRefundPushButton_slot);
  connect(AbstractGUI->PalletShipmentPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::palletShipmentPushButton_slot);

  // Стикеры
  connect(AbstractGUI->PrintTransponderStickerPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::printTransponderStickerPushButton_slot);
  connect(AbstractGUI->PrintBoxStickerPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::printBoxStickerPushButton_slot);
  connect(AbstractGUI->PrintPalletStickerPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::printPalletStickerPushButton_slot);
  ...
  // Сохранение настроек
  connect(AbstractGUI->ApplySettingsPushButton,
	  &QPushButton::clicked,
	  this,
	  &MainWindowKernel::applySettingsPushButton_slot);

  // Подключаем логгер
  connect(Logger->getWidgetLogger(),
	  &WidgetLogBackend::displayLog_signal,
	  AbstractGUI,
	  &MainWindowGUI::displayLog);
  connect(Logger->getWidgetLogger(),
	  &WidgetLogBackend::clearLogDisplay_signal,
	  AbstractGUI,
...
void MainWindowKernel::createLoggerInstance()
{
  ...
  connect(this, &MainWindowKernel::applySettings_signal, Logger, &LogSystem::applySettings);
  connect(this, &MainWindowKernel::loggerClear_signal, Logger, &LogSystem::clear);
  connect(this, &MainWindowKernel::logging, Logger, &LogSystem::generate);

  LoggerThread = new QThread(this);
  connect(LoggerThread, &QThread::finished, LoggerThread, &QThread::deleteLater);
...
void MainWindowKernel::createManagerInstance()
{
  ...
  connect(Manager, &AdminManager::notifyUser, Interactor, &InteractionSystem::generateMessage);
  connect(Manager,
	  &AdminManager::notifyUserAboutError,
	  Interactor,
	  &InteractionSystem::generateErrorMessage,
	  Qt::QueuedConnection);
  connect(Manager,
	  &AdminManager::operationPerfomingStarted,
	  Interactor,
	  &InteractionSystem::startOperationProgressDialog);
  connect(Manager,
	  &AdminManager::operationPerformingFinished,
	  Interactor,
	  &InteractionSystem::finishOperationProgressDialog);

  // Подключаем функционал
  connect(this, &MainWindowKernel::applySettings_signal, Manager, &AdminManager::applySettings);

  connect(this, &MainWindowKernel::connectDatabase_signal, Manager, &AdminManager::connectDatabase);
  connect(this,
	  &MainWindowKernel::disconnectDatabase_signal,
	  Manager,
	  &AdminManager::disconnectDatabase);
  connect(this,
	  &MainWindowKernel::showDatabaseTable_signal,
	  Manager,
	  &AdminManager::showDatabaseTable);
  connect(this,
	  &MainWindowKernel::clearDatabaseTable_signal,
	  Manager,
	  &AdminManager::clearDatabaseTable);
  connect(this,
	  &MainWindowKernel::performCustomRequest_signal,
	  Manager,
	  &AdminManager::performCustomRequest);

  connect(this, &MainWindowKernel::createNewOrder_signal, Manager, &AdminManager::createNewOrder);
  connect(this,
	  &MainWindowKernel::startOrderAssembling_signal,
	  Manager,
	  &AdminManager::startOrderAssembling);
  connect(this,
	  &MainWindowKernel::stopOrderAssembling_signal,
	  Manager,
	  &AdminManager::stopOrderAssembling);
  connect(this, &MainWindowKernel::deleteLastOrder_signal, Manager, &AdminManager::deleteLastOrder);
  connect(this, &MainWindowKernel::showOrderTable_signal, Manager, &AdminManager::showOrderTable);

  connect(this,
	  &MainWindowKernel::createNewProductionLine_signal,
	  Manager,
	  &AdminManager::createNewProductionLine);
  connect(this,
	  &MainWindowKernel::allocateInactiveProductionLines_signal,
	  Manager,
	  &AdminManager::allocateInactiveProductionLines);
  connect(this,
	  &MainWindowKernel::shutdownAllProductionLines_signal,
	  Manager,
	  &AdminManager::shutdownAllProductionLines);
  connect(this,
	  &MainWindowKernel::deleteLastProductionLine_signal,
	  Manager,
	  &AdminManager::deleteLastProductionLine);
  connect(this,
	  &MainWindowKernel::showProductionLineTable_signal,
	  Manager,
	  &AdminManager::showProductionLineTable);
  connect(this,
	  &MainWindowKernel::linkProductionLineWithBox_signal,
	  Manager,
	  &AdminManager::linkProductionLineWithBox);

  connect(this,
	  &MainWindowKernel::releaseTransponder_signal,
	  Manager,
	  &AdminManager::releaseTransponder);
  connect(this,
	  &MainWindowKernel::confirmTransponderRelease_signal,
	  Manager,
	  &AdminManager::confirmTransponderRelease);
  connect(this,
	  &MainWindowKernel::rereleaseTransponder_signal,
	  Manager,
	  &AdminManager::rereleaseTransponder);
  connect(this,
	  &MainWindowKernel::confirmTransponderRerelease_signal,
	  Manager,
	  &AdminManager::confirmTransponderRerelease);
  connect(this,
	  &MainWindowKernel::rollbackProductionLine_signal,
	  Manager,
	  &AdminManager::rollbackProductionLine);
  connect(this,
	  &MainWindowKernel::printBoxStickerOnServer_signal,
	  Manager,
	  &AdminManager::printBoxStickerOnServer);
  connect(this,
	  &MainWindowKernel::printLastBoxStickerOnServer_signal,
	  Manager,
	  &AdminManager::printLastBoxStickerOnServer);
  connect(this,
	  &MainWindowKernel::printPalletStickerOnServer_signal,
	  Manager,
	  &AdminManager::printPalletStickerOnServer);
  connect(this,
	  &MainWindowKernel::printLastPalletStickerOnServer_signal,
	  Manager,
	  &AdminManager::printLastPalletStickerOnServer);

  connect(this,
	  &MainWindowKernel::releaseTranspondersManually_signal,
	  Manager,
	  &AdminManager::releaseTranspondersManually);
  connect(this,
	  &MainWindowKernel::refundTranspondersManually_signal,
	  Manager,
	  &AdminManager::refundTranspondersManually);
  connect(this, &MainWindowKernel::shipPallets_signal, Manager, &AdminManager::shipPallets);

  connect(this, &MainWindowKernel::initIssuers_signal, Manager, &AdminManager::initIssuers);
  connect(this,
	  &MainWindowKernel::initTransportMasterKeys_signal,
	  Manager,
	  &AdminManager::initTransportMasterKeys);
  connect(this,
	  &MainWindowKernel::linkIssuerWithMasterKeys_signal,
	  Manager,
	  &AdminManager::linkIssuerWithMasterKeys);

  connect(this,
	  &MainWindowKernel::printTransponderSticker_signal,
	  Manager,
	  &AdminManager::printTransponderSticker);
  connect(this, &MainWindowKernel::printBoxSticker_signal, Manager, &AdminManager::printBoxSticker);
  connect(this,
	  &MainWindowKernel::printPalletSticker_signal,
	  Manager,
	  &AdminManager::printPalletSticker);
  connect(this,
	  &MainWindowKernel::execPrinterStickerCommandScript_signal,
	  Manager,
	  &AdminManager::execPrinterStickerCommandScript);

  // Запросы на отображение
  connect(Manager,
	  &AdminManager::displayFirmware_signal,
	  this,
	  &MainWindowKernel::displayFirmware_slot);
  connect(Manager,
	  &AdminManager::displayTransponderData_signal,
	  this,
  ...
  connect(ManagerThread, &QThread::finished, Manager, &AdminManager::deleteLater);
  connect(ManagerThread, &QThread::started, Manager, &AdminManager::insctanceThreadStarted_slot);
...
void MainWindowKernel::createInteractorInstance()
{
  ...
  connect(Interactor, &InteractionSystem::logging, Logger, &LogSystem::generate);
  connect(this,
	  &MainWindowKernel::applySettings_signal,
	  Interactor,
